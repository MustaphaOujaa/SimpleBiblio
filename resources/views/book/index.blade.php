...
<a href="{{ route('addBook') }}"
   class="inline-block px-5 py-2 text-white bg-blue-600 font-semibold rounded-lg
          hover:bg-blue-700 transition duration-300">
   Créer un livre
</a>

<h2 class="mt-4 text-3xl font-extrabold text-gray-900 sm:text-4xl">Parcourir les livres</h2>

<!-- Triage / Sorting -->
<div class="mt-6 flex justify-center w-full max-w-sm">
    <form method="GET" action="{{ route('bookIndex') }}" class="w-full flex items-center gap-3 bg-gray-50 p-2 rounded-lg border border-gray-200">
        <label for="sort" class="text-sm font-medium text-gray-600 whitespace-nowrap ml-2">Trier par:</label>
        <select name="sort" id="sort" onchange="this.form.submit()" class="block w-full border-gray-300 py-2 pl-3 pr-10 text-gray-900 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md shadow-sm bg-white cursor-pointer outline-none transition">
            <option value="latest" {{ request('sort') == 'latest' ? 'selected' : '' }}>Les plus récents</option>
            <option value="asc" {{ request('sort') == 'asc' ? 'selected' : '' }}>De A à Z (Titre)</option>
            <option value="desc" {{ request('sort') == 'desc' ? 'selected' : '' }}>De Z à A (Titre)</option>
